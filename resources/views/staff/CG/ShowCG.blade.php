    ...
    <x-card>
        <x-slot name="header">
            <div
                ...
                <h4 class="mb-0">รายงานผลการปฏิบัติงานผู้ดูแลผู้สูงอายุ (CG)</h4>
                <div class="d-flex flex-wrap gap-2 align-items-center">
                    <form action="{{ route('cg.index') }}" method="GET" class="d-flex gap-2 align-items-center me-2">
                        <input type="date" name="date" class="form-control form-control-sm" value="{{ request('date') }}">
                        <button type="submit" class="btn btn-sm btn-dark mb-0">ค้นหา</button>
                        @if(request('date'))
                            <a href="{{ route('cg.index') }}" class="btn btn-sm btn-link mb-0 text-secondary">ล้าง</a>
                        @endif
                    </form>
                    <a href="{{ route('cg.create') }}" class="btn btn-primary btn-sm mb-0">
                        <i class="fas fa-plus"></i> เพิ่ม CG
                    </a>
                    <a href="{{ route('report.all.cg') }}" target="_blank" class="btn btn-success btn-sm mb-0">
                        <i class="fas fa-print"></i>
                    </a>
                    <a href="{{ route('cg.export') }}" class="btn btn-primary btn-sm mb-0">Export Excel</a>
                </div>
            </div>
        </x-slot>
